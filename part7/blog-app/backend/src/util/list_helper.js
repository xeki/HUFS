...
const groupBlogsByAuthor = (blogs) => {
  return blogs.reduce((acc, blog) => {
    if (acc[blog.author]) {
      ...
      acc[blog.author] = {blogsCount: 1, likes: blog.likes, userId: blog.user._id}
    }
    return acc
  }, {})
}

const groupBlogsByUsers = (blogs) => {
  return blogs.reduce((acc, blog) => {
    if (acc[blog.user.userName]) {
      acc[blog.user.userName].blogsCount += 1
      acc[blog.user.userName].likes += blog.likes
    } else {
      acc[blog.user.userName] = {blogsCount: 1, likes: blog.likes, userId: blog.user._id, name: blog.user.name}
    }
    return acc
  }, {})
}
